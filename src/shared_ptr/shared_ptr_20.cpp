...
int main(int argc, char *argv[]) {
  ...
  std::thread t1([&shared_string] {
    shared_string.store(
        std::make_shared<std::string>(*shared_string.load() + "One"));
  });

  std::thread t2([&shared_string] {
    shared_string.store(
        std::make_shared<std::string>(*shared_string.load() + "Two"));
  });

  std::thread t3([&shared_string]() {
    shared_string.store(
        std::make_shared<std::string>(*shared_string.load() + "Three"));
  });

  std::thread t4([&shared_string]() {
    shared_string.store(
        std::make_shared<std::string>(*shared_string.load() + "Four"));
  });

  std::thread t5([&shared_string]() {
    shared_string.store(
        std::make_shared<std::string>(*shared_string.load() + "Five"));
  });
